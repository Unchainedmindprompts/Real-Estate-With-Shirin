...
const ORGANIZATION_ID = 'https://www.jacklinrealestate.com/#organization'
  ...
  for (const block of extractJsonLd(html)) {
    ...
    walk(block, (node) => {
      ...
      if (types.some((t) => ARTICLE_TYPES.has(t))) {
        ...
        if (!publisher || publisher['@type'] !== 'Organization' || publisher['@id'] !== BUSINESS_ID || publisher.name !== BRAND_NAME) {
          fail('ARTICLE_PUBLISHER_STUB', `${rel} article publisher must be Organization + name + ${BUSINESS_ID}`)
        }
      }
    })
  }
...
if (brandDefs.length !== 1) {
  ...
  if (!typesOf(brand).includes('Organization')) {
    fail('BRAND_TYPE', `brand @type is ${JSON.stringify(brand['@type'])}; expected Organization`)
  }
  if (typesOf(brand).includes('RealEstateAgent')) {
    fail('BRAND_TYPED_AS_AGENT', 'brand must not be typed RealEstateAgent — that collapses Person and brand')
  }
  if (brand.founder?.['@id'] !== AGENT_ID) {
    fail('BRAND_FOUNDER', `brand.founder must point at ${AGENT_ID}`)
  }
  const memberId = brand.memberOf?.['@id']
  if (memberId !== ORGANIZATION_ID) {
